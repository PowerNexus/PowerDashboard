...
**La V1 est terminée, à trois points près** (PLAN §12.4) : les sauvegardes S3 ne sont pas branchées (décision 3, défaut), et les décisions 4 (bouton Google) et 6 (version de Wings) restent à trancher. Les runners hébergés de GitHub Actions sont suspendus (facture impayée) : les workflows visent un runner auto-hébergé (`docs/runner-auto-heberge.md`, variable `CI_RUNNER` pour changer de cible). Tant qu'aucun n'est enregistré, les vérifications se font en local — `pnpm lint && pnpm typecheck && DATABASE_URL=… pnpm test && pnpm db:check && pnpm build`, puis `pnpm e2e`.
