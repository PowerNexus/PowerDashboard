...
- **Releases** : `release.yml` (étiquette `v*`) rejoue `ci.yml`, compile, assemble (`infra/release/assembler.sh`) et publie l'archive compilée. Côté serveur, toutes les commandes d'exploitation sont sous `app:` (`infra/prod/app.sh`) : `app:install`, `app:setup`, `app:start|stop|restart|status|logs`, `app:admin`, `app:password`, `app:wings`, `app:release`, `app:help`. Jamais de script sans préfixe portant le nom d'une commande de pnpm (`setup`, `restart`…) : pnpm lance la sienne. Le contrôle de fin de `deploy.sh` cherche `>Une erreur est survenue<` : le texte seul est dans le catalogue embarqué de chaque page.
