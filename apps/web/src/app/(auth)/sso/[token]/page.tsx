 ...
 * aussitôt à l'accueil du panel. Ce qu'on voit ici est donc toujours un échec —
 * lien expiré, déjà employé, ou recopié à la main.
 */
export default async function BillingSsoPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { error } = await consumeBillingLink(token);
